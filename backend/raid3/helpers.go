...
// updateParticleBackup is a server-side copy of one particle before Object.Update.
// Parallel Update on even/odd/parity can leave a mix of new and old shards; deleting
// only the successful particles (rollbackPut) breaks reconstruction. Restoring from
// these backups preserves the pre-update striping set.
type updateParticleBackup struct {
	backend    fs.Fs
	origRemote string
	backupObj  fs.Object
}

func updateBackupRemote(origRemote, tag, shard string) string {
	return origRemote + ".raid3ubak-" + tag + ".tmp." + shard
}

func randomUpdateBackupTag() string {
	var b [6]byte
...
// createUpdateParticleBackups copies each existing particle to a temp name on the same backend.
// parityObj may be nil (no parity particle yet).
func createUpdateParticleBackups(ctx context.Context, evenFs, oddFs, parityFs fs.Fs, evenObj, oddObj, parityObj fs.Object) ([]updateParticleBackup, error) {
	tag := randomUpdateBackupTag()
	var out []updateParticleBackup

	evenOrig := evenObj.Remote()
	eb, err := copyParticle(ctx, evenFs, evenObj, updateBackupRemote(evenOrig, tag, "even"))
	if err != nil {
		return nil, fmt.Errorf("even particle backup failed: %w", err)
	}
	out = append(out, updateParticleBackup{backend: evenFs, origRemote: evenOrig, backupObj: eb})

	oddOrig := oddObj.Remote()
	ob, err := copyParticle(ctx, oddFs, oddObj, updateBackupRemote(oddOrig, tag, "odd"))
	if err != nil {
		_ = eb.Remove(ctx)
		return nil, fmt.Errorf("odd particle backup failed: %w", err)
	}
	out = append(out, updateParticleBackup{backend: oddFs, origRemote: oddOrig, backupObj: ob})

	if parityObj != nil {
		pOrig := parityObj.Remote()
		pb, err := copyParticle(ctx, parityFs, parityObj, updateBackupRemote(pOrig, tag, "parity"))
		if err != nil {
			_ = eb.Remove(ctx)
			_ = ob.Remove(ctx)
			return nil, fmt.Errorf("parity particle backup failed: %w", err)
		}
		out = append(out, updateParticleBackup{backend: parityFs, origRemote: pOrig, backupObj: pb})
	}
	return out, nil
}

// restoreUpdateParticleBackups copies each backup over the live particle key. Returns the first
// error if any copy fails (caller must not delete backups in that case).
func restoreUpdateParticleBackups(ctx context.Context, backups []updateParticleBackup) error {
	var firstErr error
	for _, b := range backups {
		if _, err := copyParticle(ctx, b.backend, b.backupObj, b.origRemote); err != nil {
			fs.Errorf(b.backend, "Update rollback: failed to restore particle %q from backup: %v", b.origRemote, err)
			if firstErr == nil {
				firstErr = err
			}
		}
...
func removeUpdateParticleBackupObjects(ctx context.Context, backups []updateParticleBackup) {
	for _, b := range backups {
		if err := b.backupObj.Remove(ctx); err != nil {
			fs.Debugf(b.backend, "Update: failed to remove backup object %q: %v", b.backupObj.Remote(), err)
		}
	}
}

// removeParityObjectBestEffort deletes the parity particle if present (used when Update
// may have created parity after there was none, then failed).
func removeParityObjectBestEffort(ctx context.Context, parityFs fs.Fs, remote string) {
	if parityFs == nil {
		return
	}
	obj, err := parityFs.NewObject(ctx, remote)
	if err != nil {
		return
	}
	if err := obj.Remove(ctx); err != nil {
		fs.Debugf(parityFs, "Update rollback: could not remove parity particle %q: %v", remote, err)
	}
}
