...
run_update_fail_scenario() {
  ...
  if [[ "${is_sftp_backend}" -eq 1 ]]; then
    ...
    while (( retries > 0 )); do
      ...
      log_info "scenario:update-fail-${backend}" "Waiting for backend port ${port} to close..."
      sleep 1
      ((retries--))
    done
    if nc -z localhost "${port}" >/dev/null 2>&1; then
      log_warn "scenario:update-fail-${backend}" "Backend port ${port} still open after stop attempt."
    else
      log_info "scenario:update-fail-${backend}" "Backend port ${port} confirmed closed."
    fi
  elif [[ "${is_minio_backend}" -eq 1 ]]; then
    local port
    case "${backend}" in
