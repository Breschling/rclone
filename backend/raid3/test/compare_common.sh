...
# use_unsigned_payload: copy/upload uses async/accounting readers; AWS SigV4 default needs Seek on the body.
[${MINIO_EVEN_REMOTE}]
type = s3
provider = Minio
env_auth = false
...
wait_for_minio_port() {
  local port="$1"
  local retries="${MINIO_PORT_READY_RETRIES:-90}"
  local delay="${MINIO_PORT_READY_DELAY_SEC:-1}"
  while (( retries > 0 )); do
...
wait_for_minio_backend_ready() {
  ...
  local retries="${MINIO_API_READY_RETRIES:-45}"
  local delay="${MINIO_API_READY_DELAY_SEC:-1}"
  local max_delay="${MINIO_API_READY_MAX_DELAY_SEC:-5}"
  ...
  export RCLONE_TEST_TIMEOUT="${MINIO_READINESS_TIMEOUT:-15}"
  ...
  while (( retries > 0 )); do
    ...
    # Unknown/non-transient errors: keep retrying for readiness window.
    # This avoids false-ready transitions that later fail under load.
    sleep "${delay}"
...
ensure_minio_containers_ready() {
  ...
  # Validate all raid3 backends; for minio storage also validate single backend.
