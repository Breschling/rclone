...
Options:
  ...
  --force-config      Regenerate tests.config from compare_common.sh even if it already exists
...
if [[ -f "${CONFIG_FILE}" ]]; then
  if ! grep -q "^\[localminioraid3\]" "${CONFIG_FILE}"; then
    ...
    NEEDS_REGENERATION=1
  elif ! grep -q "use_unsigned_payload" "${CONFIG_FILE}"; then
    log_info "setup" "Config file missing use_unsigned_payload on MinIO S3 remotes - will regenerate"
    NEEDS_REGENERATION=1
  fi
fi

# Use the create_rclone_config function from common.sh
# Call with force=1 if file doesn't exist, needs regeneration, RAID3_COMPRESSION is set, or --force-config
if [[ ${NEEDS_REGENERATION} -eq 1 || -n "${RAID3_COMPRESSION:-}" || "${SETUP_FORCE_CONFIG}" -eq 1 ]]; then
